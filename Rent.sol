...
interface IVaultManager{
    ...
    event Rent(uint _lendItemID,address _NFTaddr,uint _tokenID, address _renter,uint _starttime,uint _endtime, uint8 _gameBonus,uint _rentTimes);
...
abstract contract MultiSigWallet is IMultiSigWallet{
	// 交易发起者
...
contract VaultManager is IVaultManager,Ownable,ReentrancyGuard,MultiSigWallet{
    using SafeMath for uint256;
